...
internal class OppdaterNarmesteLederServiceTest {
    ...
    @Test
    internal fun `Oppdaterer nærmeste leder hvis finnes fra før`() {
        ...
        runBlocking {
            ...
            nlListe[0].fnr shouldBeEqualTo sykmeldtFnr
            nlListe[0].narmesteLederFnr shouldBeEqualTo fnrLeder
            nlListe[0].aktivFom shouldBeEqualTo
    ...
    @Test
    internal fun `Deaktiverer tidligere nærmeste leder hvis ny leder meldes inn for samme orgnummer`() {
        testDb.connection.lagreNarmesteleder(
            orgnummer = "orgnummer",
            fnr = sykmeldtFnr,
            fnrNl = fnrLeder,
        ...
        runBlocking {
            ...
            gammelNl?.aktivFom shouldBeEqualTo
                OffsetDateTime.now(ZoneOffset.UTC).minusYears(1).toLocalDate()
            ...
            coVerify(exactly = 1) {
                narmesteLederLeesahProducer.send(
                    match { it.narmesteLederFnr == fnrLeder && it.status == DEAKTIVERT_NY_LEDER }
                )
            }
            coVerify(exactly = 1) {
                narmesteLederLeesahProducer.send(
                    match { it.narmesteLederFnr == fnrLeder2 && it.status == NY_LEDER }
                )
            }
        }
    }
    ...
    @Test
    internal fun `Kan ha to aktive NL samtidig hvis ulikt orgnummer`() {
        testDb.connection.lagreNarmesteleder(
            orgnummer = "orgnummer",
            fnr = sykmeldtFnr,
            fnrNl = fnrLeder,
        ...
        runBlocking {
            ...
            nlOrgnummer1?.aktivFom shouldBeEqualTo
                OffsetDateTime.now(ZoneOffset.UTC).minusYears(1).toLocalDate()
    ...
    @Test
    internal fun `Feiler hvis ansatt ikke finnes i PDL`() {
        coEvery { pdlPersonService.getPersoner(any(), any()) } returns
            mapOf(
                Pair(fnrLeder, PdlPerson(Navn("Leder", null, "Ledersen"), fnrLeder, "aktorid")),
    ...
    @Test
    internal fun `Feiler hvis nærmeste leder ikke finnes i PDL`() {
        coEvery { pdlPersonService.getPersoner(any(), any()) } returns
            mapOf(
                Pair(fnrLeder, null),
    ...
    @Test
    internal fun `NlAvbrutt deaktiverer nærmeste leder og ber om ny NL hvis arbeidsforholdet er aktivt`() {
        coEvery { arbeidsgiverService.getArbeidsgivere(any(), any(), any()) } returns
            listOf(Arbeidsgiverinfo("orgnummer", "juridiskOrgnummer", aktivtArbeidsforhold = true))
        testDb.connection.lagreNarmesteleder(
            orgnummer = "orgnummer",
            fnr = sykmeldtFnr,
            fnrNl = fnrLeder,
    ...
    @Test
    internal fun `NlAvbrutt deaktiverer nærmeste leder av leder og ber om ny NL hvis arbeidsforholdet er aktivt`() {
        coEvery { arbeidsgiverService.getArbeidsgivere(any(), any(), any()) } returns
            listOf(Arbeidsgiverinfo("orgnummer", "juridiskOrgnummer", aktivtArbeidsforhold = true))
        testDb.connection.lagreNarmesteleder(
            orgnummer = "orgnummer",
    ...
    @Test
    internal fun `NlAvbrutt deaktiverer nærmeste leder av arbeidstaker`() {
        coEvery { arbeidsgiverService.getArbeidsgivere(any(), any(), any()) } returns
            listOf(Arbeidsgiverinfo("orgnummer", "juridiskOrgnummer", aktivtArbeidsforhold = true))
        testDb.connection.lagreNarmesteleder(
            orgnummer = "orgnummer",
    ...
    @Test
    internal fun `NlAvbrutt deaktiverer nærmeste leder av arbeidstaker og ber ikke om ny leder hvis arbeidsforholdet er inaktivt`() {
        coEvery { arbeidsgiverService.getArbeidsgivere(any(), any(), any()) } returns
            listOf(Arbeidsgiverinfo("orgnummer", "juridiskOrgnummer", aktivtArbeidsforhold = false))
        testDb.connection.lagreNarmesteleder(
            orgnummer = "orgnummer",
            fnr = sykmeldtFnr,
            fnrNl = fnrLeder,
    ...
    @Test
    internal fun `NlAvbrutt deaktiverer nærmeste leder av arbeidstaker og ber ikke om ny leder hvis arbeidsforhold ikke finnes i listen`() {
        coEvery { arbeidsgiverService.getArbeidsgivere(any(), any(), any()) } returns
            listOf(
                Arbeidsgiverinfo("orgnummer2", "juridiskOrgnummer", aktivtArbeidsforhold = false)
            )
        testDb.connection.lagreNarmesteleder(
            orgnummer = "orgnummer",
            fnr = sykmeldtFnr,
    ...
    @Test
    internal fun `NlAvbrutt feiler ikke hvis det ikke er noen ledere å deaktivere, og påvirker ikke nl for andre arbeidsforhold`() {
        testDb.connection.lagreNarmesteleder(
